...
### Expectations that the site will meet:
...
## Target Audience
- The target audience will be quite a wide range of adults and children, as horse riding can be a hobby for people of all ages. Although children usually go horse riding with their parents/guardians so the site will be aimed towards adults and families with children, more than towards just children themselves. The site will include images and nice coloured backgrounds as well as bold, easy to read text to appeal to younger ages. At the same time the site will not be overly colourful to keep a professional look which will appeal to parents and adults.
...
- **Color Scheme:** I decided to use a colour scheme of green and white to match the colour of country side and nature around the location where the riding school would be based. Green fields are a common location for horses and white stands out well against it making for a calming and natural look. I considered using blue to represent the blue sky but it doesn't contrast well enough with green.

- **Font:** I used the fonts 'Open Sans' and 'Noto Sans Japanese' as they compliment each other well with their different weights. I used a heaveir weight for the logo so it would stand out. I kept the paragraphs in the same lighter weight and style, and the h2 elements in the same font with a slightly heavier weight so they would all be easy to distinguish.

### *Structural Features:*

### The Header

- **Logo:** The logo is white which follows the colour scheme for all text on this site. It also has a heavier font-weight to help it stand out as a logo should. It can be clicked on to navigate the user back to the main page, from the Gallery page for example.
...
- A section about prices is also a feature that could be added. This is something any customer would most likely want to know before they book. Making the Sign Up section into a separate page entirely could be a good idea for this. Then all of these future features could go together nicely on that page. For example the page could have a schedule, prices list, booking form and map in that order. It would be logical to have all booking related information on one page.

- As far as the actual code goes I would love to add a flexbox to the text in the About section as it would make the code clearer and easier to reuse. It would also be easier to align the text nicely on the page if any changes were made to it. At the time I didn't know how to use flexbox, I learnt it when I was trying to align my images in the Rides section just after. I realised it would have been ideal to implement it into the About section, and could even be used to align the header and footer elements too. Therefore it is definitely something I would like to add more of to my site and I will be using it a lot more in future projects too.
...
- When the user first opens the website the main page is shown, with the logo, navigation menu and hero-image. The navigation menu allows the user to navigate to the Rides section, Gallery and Sign Up section.
...
- When the form is complete and the Sign Up button clicked, the user will be taken to the confirmation page.
